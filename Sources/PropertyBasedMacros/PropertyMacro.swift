...
public struct PropertyMacro: PeerMacro {
    public static func expansion(of node: AttributeSyntax, providingPeersOf declaration: some DeclSyntaxProtocol, in context: some MacroExpansionContext) throws -> [DeclSyntax] {
        ...
        if function.attributes.first(where: { $0.attribute?.name.name == "Test" }) != nil {
        ...
        let forwardedArgs = macroArguments[..<inputIndex].dropLast(countArg != nil ? 1 : 0).dropFirst(nameArg != nil ? 1 : 0)
        ...
        return [
            """
            @Test(\(testName)\(raw: forwardedTraits))
            func \(name)() async {
    ...
    public enum PropertyMacroError: Error, CustomStringConvertible {
        ...
        public var description: String {
            switch(self) {
            case .notAFunction:
                "Macro @Property must be set on a function declaration"
            case .redundantTest:
                "@Property attribute cannot be combined with @Test."
            }
        }
    }
}
