...
public struct PropertyMacro: PeerMacro {
    public static func expansion(
        ...
        if let specifier = function.parameters.compactMap({ $0.unsupportedSpecifier }).first {
            throw PropertyMacroError.invalidParameterSpecifier(specifier)
        }
        ...
        if !function.returnsVoid {
            let warning = DiagnosticBuilder(for: function._syntax)
                .severity(.warning)
                .message("The result of this function will be discarded during testing").build()
    ...
    public enum PropertyMacroError: Error, CustomStringConvertible {
        case notAFunction
        case redundantTest
        case invalidParameterSpecifier(String)
        ...
        public var description: String {
            ...
            case .invalidParameterSpecifier(let label):
                "Attribute 'PropertyTest' cannot be applied to a function with a parameter marked '\(label)'"
            }
        }
    }
}
