...
extension Shrink {
    ...
    public static func omitSingleElement<Element, Base: RangeReplaceableCollection<Element>>(collection: Base, lowerBound: Int = 0) -> (some Sequence<Base>)? {
        return collection.count > lowerBound ? collection.indices.lazy.map {
            Base(collection[..<$0] + collection[collection.index(after: $0)...])
        } : nil
    }
    
    public static func shrinkArray<Item, Collection: RangeReplaceableCollection<Item> & MutableCollection<Item>>(_ array: Collection, shrinker: @escaping Shrinker<Item, some Sequence<Item>>, lowerBound: Int = 0) -> some Sequence<Collection> {
