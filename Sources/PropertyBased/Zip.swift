...
> {
    return .init(
        run: { rng, limit in
            try (
                p0.runFull(&rng, limit).input,
                p1.runFull(&rng, limit).input,
            )
        },
        shrink: { tuple in
    ...
    return .init(
        run: { rng, limit in
            try (
                ...
                p3.runFull(&rng, limit).input,
                p4.runFull(&rng, limit).input,
                p5.runFull(&rng, limit).input,
            )
        },
        shrink: { tuple in
            Shrink.shrinkTuple(
                tuple, shrinkers: p0._shrinker, p1._shrinker, p2._shrinker, p3._shrinker, p4._shrinker, p5._shrinker)
        },
        finalResult: { input in
            guard
                let r0 = p0._mapFilter(input.0),
    ...
    return .init(
        run: { rng, limit in
            try (
                ...
                p5.runFull(&rng, limit).input,
                p6.runFull(&rng, limit).input,
                p7.runFull(&rng, limit).input,
            )
        },
        shrink: { tuple in
            Shrink.shrinkTuple(
                tuple, shrinkers: p0._shrinker, p1._shrinker, p2._shrinker, p3._shrinker, p4._shrinker, p5._shrinker,
                p6._shrinker, p7._shrinker)
        },
        finalResult: { input in
            guard
                let r0 = p0._mapFilter(input.0),
    ...
    return .init(
        run: { rng, limit in
            try (
                ...
                p6.runFull(&rng, limit).input,
                p7.runFull(&rng, limit).input,
                p8.runFull(&rng, limit).input,
            )
        },
        shrink: { tuple in
            Shrink.shrinkTuple(
                tuple, shrinkers: p0._shrinker, p1._shrinker, p2._shrinker, p3._shrinker, p4._shrinker, p5._shrinker,
                p6._shrinker, p7._shrinker, p8._shrinker)
        },
        finalResult: { input in
            guard
                let r0 = p0._mapFilter(input.0),
    ...
    return .init(
        run: { rng, limit in
            try (
                ...
                p7.runFull(&rng, limit).input,
                p8.runFull(&rng, limit).input,
                p9.runFull(&rng, limit).input,
            )
        },
        shrink: { tuple in
            Shrink.shrinkTuple(
                tuple, shrinkers: p0._shrinker, p1._shrinker, p2._shrinker, p3._shrinker, p4._shrinker, p5._shrinker,
                p6._shrinker, p7._shrinker, p8._shrinker, p9._shrinker)
        },
        finalResult: { input in
            guard
                let r0 = p0._mapFilter(input.0),
