...
extension Gen where Value: BinaryFloatingPoint & Sendable, Value.RawSignificand: FixedWidthInteger {
    ...
    @inlinable
    public static func float(in range: ClosedRange<Value>) -> Generator<Shrink.FloatingPointShrinkSequence<Value>, Value> {
        return .init(
    ...
    /// - Returns: A generator of random values within the bounds of range.
    @inlinable
    public static func float(in range: Range<Value>) -> Generator<Shrink.FloatingPointShrinkSequence<Value>, Value> {
        return .init(
            run: { rng in Value.random(in: range, using: &rng) },
            shrink: { $0.shrink(within: range) }
        )
